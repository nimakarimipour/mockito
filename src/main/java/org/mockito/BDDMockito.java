...
/**
 * Behavior Driven Development style of writing tests uses <b>//given //when //then</b> comments as
 * fundamental parts of your test methods. This is exactly how we write our tests and we warmly
 * encourage you to do so!
 *
 * <p>Start learning about BDD here: <a
 * href="http://en.wikipedia.org/wiki/Behavior_Driven_Development">http://en.wikipedia.org/wiki/Behavior_Driven_Development</a>
 *
 * <p>The problem is that current stubbing api with canonical role of <b>when</b> word does not
 * integrate nicely with <b>//given //when //then</b> comments. It's because stubbing belongs to
 * <b>given</b> component of the test and not to the <b>when</b> component of the test. Hence {@link
 * BDDMockito} class introduces an alias so that you stub method calls with {@link
 * BDDMockito#given(Object)} method. Now it really nicely integrates with the <b>given</b> component
 * of a BDD style test!
 *
 * <p>Here is how the test might look like:
 *
 ...
 * </code></pre>
 *
 * <p>For BDD style mock verification take a look at {@link Then} in action:
 *
 * <pre class="code"><code class="java">
 *   person.ride(bike);
 *   person.ride(bike);
 ...
 * </code></pre>
 *
 * <p>It is also possible to do BDD style {@link InOrder} verification:
 *
 * <pre class="code"><code class="java">
 ...
 * </code></pre>
 *
 * <p>One of the purposes of BDDMockito is also to show how to tailor the mocking syntax to a
...
public class BDDMockito extends Mockito {
  ...
  private static class BDDOngoingStubbingImpl<T> implements BDDMyOngoingStubbing<T> {
    ...
    public BDDMyOngoingStubbing<T> willAnswer(Answer<?> answer) {
              ...
              return new BDDOngoingStubbingImpl<T>(Nullability.castToNonnull(mockitoOngoingStubbing, "checked for null").thenAnswer(answer));
      }
    ...
    public BDDMyOngoingStubbing<T> will(Answer<?> answer) {
              ...
              return new BDDOngoingStubbingImpl<T>(Nullability.castToNonnull(mockitoOngoingStubbing.then(answer), "explicit null check"));
    }
    ...
    public BDDMyOngoingStubbing<T> willReturn(T value) {
                ...
                return new BDDOngoingStubbingImpl<T>(Nullability.castToNonnull(mockitoOngoingStubbing, "checked if null").thenReturn(value));
    }
    ...
    public BDDMyOngoingStubbing<T> willReturn(T value, T... values) {
              ...
              return new BDDOngoingStubbingImpl<T>(Nullability.castToNonnull(mockitoOngoingStubbing, "explicitly checked for null").thenReturn(value, values));
    }
    ...
    public BDDMyOngoingStubbing<T> willThrow(Throwable... throwables) {
              ...
              return new BDDOngoingStubbingImpl<T>(Nullability.castToNonnull(mockitoOngoingStubbing, "checked for null").thenThrow(throwables));
    }
    ...
    public BDDMyOngoingStubbing<T> willThrow(Class<? extends Throwable> throwableType) {
                ...
                return new BDDOngoingStubbingImpl<T>(Nullability.castToNonnull(mockitoOngoingStubbing, "checked for null").thenThrow(throwableType));
      }
    ...
    public BDDMyOngoingStubbing<T> willCallRealMethod() {
              ...
              return new BDDOngoingStubbingImpl<T>(Nullability.castToNonnull(mockitoOngoingStubbing, "checked for null"));
      }
  ...
  public interface BDDStubber {
     ...
     *
     * <p>This method will be removed in version 4.0.0
     *
     * @since 1.8.0
