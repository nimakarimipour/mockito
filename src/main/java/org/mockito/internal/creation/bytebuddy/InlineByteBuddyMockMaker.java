 ...
 *
 * <p>This mock maker which uses a combination of the Java instrumentation API and sub-classing
 * rather than creating a new sub-class to create a mock. This way, it becomes possible to mock
 * final types and methods. This mock maker <strong>must to be activated explicitly</strong> for
 * supporting mocking final types and methods:
 *
 * <p>
 *
 * <p>This mock maker can be activated by creating the file <code>
 * /mockito-extensions/org.mockito.plugins.MockMaker</code> containing the text <code>
 * mock-maker-inline</code> or <code>
 * org.mockito.internal.creation.bytebuddy.InlineByteBuddyMockMaker</code>.
 *
 * <p>
 *
 * <p>This mock maker will make a best effort to avoid subclass creation when creating a mock.
 * Otherwise it will use the <code>
 * org.mockito.internal.creation.bytebuddy.SubclassByteBuddyMockMaker</code> to create the mock
 * class. That means that the following condition is true
 *
 * <p>
 *
 ...
 * </pre></code>
 *
 * <p>unless any of the following conditions is met, in such case the mock maker <em>fall backs</em>
 * to the the creation of a subclass.
 *
 * <p>
 *
 * <ul>
 *   <li>the type to mock is an abstract class.
 *   <li>the mock is set to require additional interfaces.
 *   <li>the mock is <a href="#20">explicitly set to support serialization</a>.
 * </ul>
 *
 * <p>
 *
 * <p>Some type of the JDK cannot be mocked, this includes <code>Class</code>, <code>String</code>,
 * and wrapper types.
 *
 * <p>
 *
 * <p>Nevertheless, final methods of such types are mocked when using the inlining mock maker.
 * Mocking final types and enums does however remain impossible when explicitly requiring
 * serialization support or when adding ancillary interfaces.
 *
 * <p>
 *
 * <p>Important behavioral changes when using inline-mocks:
 *
 * <ul>
 *   <li>Mockito is capable of mocking package-private methods even if they are defined in different
 *       packages than the mocked type. Mockito voluntarily never mocks package-visible methods
 *       within <code>java.*</code> packages.
 *   <li>Additionally to final types, Mockito can now mock types that are not visible for extension;
 *       such types include private types in a protected package.
 *   <li>Mockito can no longer mock <code>native</code> methods. Inline mocks require byte code
 *       manipulation of a method where native methods do not offer any byte code to manipulate.
 *   <li>Mockito cannot longer strip <code>synchronized</code> modifiers from mocked instances.
 * </ul>
 *
 * <p>
 *
 * <p>Note that inline mocks require a Java agent to be attached. Mockito will attempt an attachment
 * of a Java agent upon loading the mock maker for creating inline mocks. Such runtime attachment is
 * only possible when using a JVM that is part of a JDK or when using a Java 9 VM. When running on a
 * non-JDK VM prior to Java 9, it is however possible to manually add the <a
 * href="http://bytebuddy.net">Byte Buddy Java agent jar</a> using the <code>-javaagent</code>
 * parameter upon starting the JVM. Furthermore, the inlining mock maker requires the VM to support
 * class retransformation (also known as HotSwap). All major VM distributions such as HotSpot
 * (OpenJDK), J9 (IBM/Websphere) or Zing (Azul) support this feature.
 */
@Incubating
public class InlineByteBuddyMockMaker
      ...
      try {
        ...
        try {
          ...
          InputStream inputStream =
              InlineByteBuddyMockMaker.class.getClassLoader().getResourceAsStream(source + ".raw");
          if (inputStream == null) {
            throw new IllegalStateException(
                join(
  ...
  private <T> T doCreateMock(
      MockCreationSettings<T> settings, MockHandler handler, boolean nullOnNonInlineConstruction) {
  ...
  public TypeMockability isTypeMockable(final Class<?> type) {
    return new TypeMockability() {
      public boolean mockable() {
          return INSTRUMENTATION != null && INSTRUMENTATION.isModifiableClass(type) && !EXCLUDES.contains(type);
        }
              ...
              (object, context) -> {
                ...
                MockMethodInterceptor interceptor =
                    new MockMethodInterceptor(
                        handlerFactory.apply(context), settingsFactory.apply(context));
