...
public class ReturnsDeepStubs implements Answer<Object>, Serializable {
   ...
   * <li>Finally as we want to mock the actual type, but we want to pass along the contextual
   *     generics meta-data that was resolved for the current return type, for this to happen we
   *     associate to the mock an new instance of {@link ReturnsDeepStubs} answer in which we will
   *     store the returned type generic metadata.
   *
  ...
  protected GenericMetadataSupport actualParameterizedType(Object mock) {
    ...
    if (handler == null) {
      throw new NotAMockException("No MockHandler found for the mock: " + mock.getClass());
    }

    CreationSettings mockSettings = (CreationSettings) handler.getMockSettings();
  ...
  private static class ReturnsDeepStubsSerializationFallback extends ReturnsDeepStubs
     ...
     *
     * <p>The issue is that GenericMetadataSupport is not serializable because the type elements
     * inferred via reflection are not serializable. Supporting serialization would require to
     * replace all types coming from the Java reflection with our own and still managing type
     * equality with the JDK ones.
     */
